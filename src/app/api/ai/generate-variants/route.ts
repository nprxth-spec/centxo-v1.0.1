...
export async function POST(request: NextRequest) {
  ...
  const { checkAiLimit, incrementAiUsage } = await import('@/lib/user-limit');
  const { allowed, reason, isTrialExpired } = await checkAiLimit(session.user.id);

  if (!allowed) {
    return NextResponse.json({ error: reason, isTrialExpired }, { status: 403 });
  }
...
- Each variant must differ in angle, tone, or CTA.`;
    ...
    const ctx = productContext || primaryText || headline || 'สินค้า';
    // Note: Fallback variants don't count towards AI usage limit
    return NextResponse.json({ variants: fallbackVariants(count, ctx) });
  }
}
