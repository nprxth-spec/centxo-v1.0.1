...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingPostId) {
      ...
      if (currency === 'USD' && dailyBudget < 500) dailyBudget = 500; // Min $5
    ...
    if (currency === 'USD' && dailyBudget < 500) {
      console.warn(`⚠ Budget ${dailyBudget} is too low for USD. Boosting to minimum 500 ($5).`);
      dailyBudget = 500;
    }
    if (dailyBudget < 50) {
