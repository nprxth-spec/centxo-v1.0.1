...
export async function GET(request: NextRequest) {
  try {
    ...
    const fetchAdAccounts = async (): Promise<AdAccountsResponse> => {
      ...
      const tokens: { token: string; name: string; isTeamMember?: boolean }[] = userTokens.map(t => ({
        token: t.token,
        name: t.name,
        isTeamMember: false
      }));

      console.log('[ad-accounts] User tokens collected:', tokens.length);
    ...
    // Collect all selected ad account IDs from active subscriptions
    const allSelectedAdAccountIds = new Set<string>();
    subscriptions.forEach((sub: any) => {
      if (sub.selectedAdAccountIds) {
        try {
          const ids = JSON.parse(sub.selectedAdAccountIds);
          if (Array.isArray(ids)) {
            ids.forEach((id: string) => allSelectedAdAccountIds.add(id));
          }
        } catch (e) {
          // Ignore parse errors
        }
    ...
    // Filter accounts: if no selection, show all (backward compatible)
    const filteredAccounts = allSelectedAdAccountIds.size > 0
      ? result.accounts.filter(acc => allSelectedAdAccountIds.has(acc.id))
      : result.accounts;
