...
export default function CreateAdsPage() {
  ...
  const [form, setForm] = useState({
    ...
    productContext: '',
    targetingType: planLimits?.aiAccess ? 'ai' : 'manual',
  ...
  // Enforce AI limit
  useEffect(() => {
    if (planLimits && !planLimits.aiAccess && form.targetingType === 'ai') {
      setForm(prev => ({ ...prev, targetingType: 'manual' }));
    }
  }, [planLimits, form.targetingType]);
  ...
  const accountCurrency = selectedAcc?.currency || 'USD';
  ...
  const handleLaunch = async () => {
    ...
    const formatCreateError = (raw: string): string => {
      ...
      if (s.includes('facebook') && s.includes('connect')) return t('createAds.error.fbConnect', 'ยังไม่ได้เชื่อมต่อ Facebook กรุณาไปที่ Account > Team');
    ...
    try {
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
          ...
          setTimeout(() => router.push('/ads?tab=campaigns&refresh=true'), 2200);
          return;
        } catch (err: any) {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-gradient-to-b from-background to-muted/30">
      <div className="mx-auto max-w-4xl px-4 py-8 md:px-6">
        <div className="mb-8">
          <h1 className="text-page-title font-outfit">
                  ...
                  <div className="space-y-6">
                    ...
                    <Tabs value={adSource} onValueChange={(v: any) => setAdSource(v)}>
                      ...
                      <TabsContent value="upload" className="mt-4 space-y-4">
                        <div
                          className={`
                            flex h-[17rem] min-h-0 cursor-pointer flex-col items-center justify-center overflow-hidden rounded-lg border-2 border-dashed transition-colors
                      ...
                      <TabsContent value="library" className="mt-4">
                        <div className="flex h-[17rem] min-h-0 flex-col overflow-hidden rounded-lg border bg-muted/20 p-4">
                ...
                {currentStep === 3 && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <Tabs value={form.targetingType} onValueChange={(v: any) => {
                        if (v === 'ai' && !planLimits?.aiAccess) return;
                        setForm((p) => ({ ...p, targetingType: v }))
                      }}>
                        <TabsList className="grid w-full grid-cols-2">
                          <TabsTrigger
                            value="ai"
                          ...
                          >
                            ...
                            {t('createAds.strategy.ai', 'AI อัตโนมัติ')}
                            {!planLimits?.aiAccess && <span className="ml-2 text-xs bg-muted px-1 rounded">PRO</span>}
                          </TabsTrigger>
                        ...
                        <TabsContent value="ai" className="rounded-lg border bg-muted/20 p-4 text-sm text-muted-foreground">
                          {!planLimits?.aiAccess ? (
                            <div className="flex flex-col items-center gap-2 text-center text-amber-600">
                              <span>{t('createAds.strategy.planNoAi')}</span>
                              <Link href="/settings?tab=payment" className="text-primary underline">{t('createAds.strategy.upgradePackage')}</Link>
                            </div>
                          ) : (
