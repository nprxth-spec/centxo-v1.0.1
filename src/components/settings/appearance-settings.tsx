...
export function AppearanceSettings() {
    ...
    useEffect(() => {
        setMounted(true);
        const saved = typeof window !== 'undefined' ? localStorage.getItem('user_timezone') : null;
        setTimezone(saved || Intl.DateTimeFormat().resolvedOptions().timeZone);
    }, []);

    const handleTimezoneChange = (tz: string) => {
        setTimezone(tz);
        if (typeof window !== 'undefined') localStorage.setItem('user_timezone', tz);
    };
            ...
            <div className="space-y-2">
                ...
                <div className="flex flex-col gap-1.5">
                    ...
                    <p className="text-[0.8rem] text-muted-foreground">
                        {t('settings.appearance.timezoneDesc', 'Used for statistics and time display. Saved when you change it.')}
                    </p>
                </div>
            </div>
