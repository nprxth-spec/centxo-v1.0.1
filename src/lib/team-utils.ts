...
/**
 * Get the effective host ID for a user.
 * If the user is a team member, returns the host's user ID.
 * Otherwise, returns the user's own ID.
 */
export async function getEffectiveHostId(userId: string, email?: string): Promise<string> {
  if (!email) {
