...
/**
 * Get insights using Marketing API (uses ads_read permission, not read_insights)
 * @param objectId - Ad account ID (act_XXX) or campaign/ad/adset ID
 * @param level - 'account', 'campaign', 'adset', or 'ad'
 * @note When objectId is an ad account ID (act_XXX), this uses Marketing API insights which require ads_read permission
 */
export async function getInsights(accessToken: string, objectId: string, level: string, dateRange?: { from: string, to: string }) {
    ...
    // Marketing API insights endpoint - uses ads_read permission (not read_insights)
